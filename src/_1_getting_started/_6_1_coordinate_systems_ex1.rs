// Ex1 - Experimenting with fov and aspect ration
// increased fov - more of the screen is visible, objects are smaller
// set constant aspect ratio 3 - while it should be 1, cubes are all shortened on the x axis (opposite happens with <1 ratio)
use std::ptr;
use std::mem;
use std::ffi::CStr;
use std::path::Path;
...
const MESSAGE : &str = "Chapter 1 : Part 6 : Ex 1 : Experimenting with fov and aspect ratio";
const WINDOW_WIDTH: u32 = 800;
const WINDOW_HEIGHT: u32 = 800;
const WINDOW_TITLE: &str = "Coordinate Systems - Ex1";
...
pub fn main_1_6_1() {
    ...
    let (mut _current_width,mut _current_height) = (WINDOW_WIDTH, WINDOW_HEIGHT);
    ...
    let default_shader = Shader::new("./src/_1_getting_started/shaders/6_0_default.vert","./src/_1_getting_started/shaders/5_0_default.frag");
    ...
    while !window.should_close() {
        ...
        for (_, event) in glfw::flush_messages(&events) {
            ...
            match event {
                glfw::WindowEvent::FramebufferSize(w, h) => unsafe {
                    gl::Viewport(0, 0, w, h);
                    // Updating aspect ratio for perspective fructrum
                    _current_width = w as u32;
                    _current_height = h as u32;
        ...
        unsafe {
            ...
            let projection = glm::perspective(3., f32::to_radians(90.), 0.1, 100.);
            default_shader.set_mat4(c_str!("view"),  view); // This can be outside the loop in this case, but view does change every frame
            default_shader.set_mat4(c_str!("projection"),  projection); // This should be outside the loop, since projection doesnt change every frame
