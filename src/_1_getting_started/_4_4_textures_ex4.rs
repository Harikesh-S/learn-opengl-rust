// Ex4 - use uniform to change mix alpha using arrow keys
...
const MESSAGE : &str = "Chapter 1 : Part 4 : Ex 4 : Change mix value using arrow keys";
const WINDOW_WIDTH: u32 = 800;
const WINDOW_HEIGHT: u32 = 800;
const WINDOW_TITLE: &str = "Textures - Ex4";
...
pub fn main_1_4_4() {
    ...
    let default_shader = Shader::new("./src/_1_getting_started/shaders/4_0_default.vert","./src/_1_getting_started/shaders/4_4_default.frag");
    ...
    // Init variables to use in loop

    let mut mix_value: GLfloat = 0.5;
    let mut prev_time = glfw.get_time();
    // --Render loop--------------------------------------------------------------------------------------------------------------- //

    while !window.should_close() {

        // Process inputs - adding time delta
        let curr_time = glfw.get_time();
        let time_delta = curr_time - prev_time;
        if time_delta > 1./60. {
            process_input(&mut window, time_delta, &mut mix_value);
            prev_time = curr_time;
        }
        ...
        unsafe {
            ...
            default_shader.use_program();
            default_shader.set_float(c_str!("mixAlpha"), mix_value);
...
fn process_input(window: &mut glfw::PWindow, time_delta : f64, mix_value: &mut GLfloat) {
    ...
    if window.get_key(glfw::Key::Up) == glfw::Action::Press {
        *mix_value += (time_delta * 0.5) as GLfloat;
        if *mix_value > 1.0 {
            *mix_value = 1.0;
        }
    }
