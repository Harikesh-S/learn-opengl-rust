...
impl TextureType {
    ...
    pub fn update_count(&self, diffuse_number : &mut i32, specular_number : &mut i32) -> i32 {
        let num;
        match self{
            Self::Diffuse => {
                num = *diffuse_number;
    ...
    pub fn draw(&self,shader : &Shader) {
        ...
        while i < self.textures.len() {
            unsafe{
                ...
                let num = self.textures[i].tex_type.update_count(&mut diffuse_number, &mut specular_number);

                // Update the texture uniform
                shader.set_int( &self.textures[i].tex_type.get_shader_name(num), self.textures[i].id as i32);
