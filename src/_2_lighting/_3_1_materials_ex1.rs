// Ex 1 : Additionally make the light cube's color change
...
const MESSAGE : &str = "Chapter 2 : Part 3 : Ex1 : Changing the light cubes color";
...
const LIGHT_LOCATION : glm::Vec3 = glm::Vec3::new(1.0, 1.0, 1.5);
...
pub fn main_2_3_1() {
    ...
    while !window.should_close() {
        ...
        // Varying the light color over time
        let light_color: glm::Vec3 = glm::vec3(
            f32::sin(glfw.get_time() as f32 * 2.0),
            f32::sin(glfw.get_time() as f32 * 0.7),
            f32::sin(glfw.get_time() as f32 * 1.3),
        );
        // reducing the intensity
        let ambient_color: glm::Vec3 = light_color.component_mul(&glm::vec3(0.2 as f32,0.2,0.2));   // a * b is not supported int nalgebra-glm
        let diffuse_color: glm::Vec3 = light_color.component_mul(&glm::vec3(0.5 as f32,0.5,0.5));   // since its used for matrix mul
        ...
        unsafe {
            ...
            // Set material
            default_shader.set_vec3_values(c_str!("material.ambient"),  0.7,0.5,0.8); 
            default_shader.set_vec3_values(c_str!("material.diffuse"),  0.7,0.5,0.8); 
            default_shader.set_vec3_values(c_str!("material.specular"),  0.7,0.5,0.8); 
            default_shader.set_float(c_str!("material.shininess"),  32.0); 
            // Light position and colors
            default_shader.set_vec3(c_str!("light.ambient"),  ambient_color);      // low because we dont want amient color to be too dominant
            default_shader.set_vec3(c_str!("light.diffuse"),  diffuse_color);      // exact color that we want
            ...
            light_shader.set_vec3(c_str!("lightColor"), light_color);     // Materials Ex1
