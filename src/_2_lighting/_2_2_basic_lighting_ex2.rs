// Ex 2 - Experimenting with various lighting strengths, shininiess
// Normal - normal lighting
// High ambient - all sides are visible regardless of light cube
// High diffuse - dark sides are not visible, specular spot is not visible
// High specular - color is only visible when looking at a specular spot
// High shininess - similar to normal, but the spot is more focussed
// Equal - similar to normal, but has more ambient light
...
const MESSAGE : &str = "Chapter 2 : Part 2 : Ex 2 : Experimenting with various lighting strengths, shininiess";
...
const CUBE_POSITIONS : [(glm::Vec3, GLfloat, GLfloat, GLfloat, GLint);6] = [
    (glm::Vec3::new(-2., 1.,0.), 0.1, 1.0, 0.5, 32),  // normal
    (glm::Vec3::new(0., 1.,0.), 1.0, 0.1, 0.1, 32),  // high ambient
    (glm::Vec3::new(2., 1.,0.), 0.1, 1.0, 0.1, 32),  // high diffuse
    (glm::Vec3::new(-2.,-1.,0.), 0.1, 0.1, 1.0, 32),  // high specular
    (glm::Vec3::new(0.,-1.,0.), 0.1, 1.0, 0.5, 256),  // high shininess
    (glm::Vec3::new(2.,-1.,0.), 0.5, 0.5, 0.5, 64),  // equal
];
...
pub fn main_2_2_2() {
    ...
    // Shader Program
    let default_shader = Shader::new("./src/_2_lighting/shaders/2_0_default.vert","./src/_2_lighting/shaders/2_2_default.frag");
    ...
    // Camera
    let mut camera : FreeCamera = FreeCamera::new(glm::vec3(-1.,0.,6.), 0., 0., -85., WINDOW_WIDTH, WINDOW_HEIGHT);
    ...
    while !window.should_close() {
        ...
        unsafe {
            ...
            // Setting required uniforms for fragment shader
            ...
            while i < CUBE_POSITIONS.len() {
                ...
                default_shader.set_float(c_str!("ambientStrength"),  CUBE_POSITIONS[i].1); 
                default_shader.set_float(c_str!("diffuseStength"),  CUBE_POSITIONS[i].2); 
                default_shader.set_float(c_str!("specularStrength"),  CUBE_POSITIONS[i].3); 
                default_shader.set_int(c_str!("shininess"),  CUBE_POSITIONS[i].4); 
